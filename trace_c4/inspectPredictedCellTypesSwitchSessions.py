...
def main(mouse_name="Ana3",
         ...
         save_dir=os.path.join(get_dropbox_path(), "AnalysisOutput/Cell_type_counts/SwitchSessions"),
    ...
    if os.path.exists(switch_folder) and os.path.isfile(os.path.join(switch_folder,"c4","c4_results_fpfnThreshold_0.1_confidenceRatio_1.5","cluster_predicted_cell_type.tsv")):
        switch_folder_name = "SwitchSessionStitching"                
        mouse_folders.append(switch_folder_name)    
    elif os.path.isfile(os.path.join(alt_directory, mouse_name,"SwitchSessionStitching","cluster_predicted_cell_type.tsv")):
        dp_base = os.path.join(alt_directory, mouse_name)
        switch_folder_name = "SwitchSessionStitching"                
        mouse_folders.append(switch_folder_name)   
    ...
    try:
        for folder in mouse_folders:
            if general_results:
                ...
                if not os.path.exists(folder_path) and os.path.exists(os.path.join(dp_base, folder)):
                    folder_path = os.path.join(dp_base, folder)
        ...
        if not total_cell_type_counts.empty:
            ...
            save_counts_to_tsv(
                ...
                total_fractions, 
                os.path.join(save_dir, f"{mouse_name}_switch_sessions_cell_type_counts_fpfnThreshold_{contamination_ratio}_confidenceRatio_{confidence_ratio_threshold}.tsv")
            )
