...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 26 08:52:02 2025
...
def plot_pie_charts(pie_chart_data, save_path):
    """Plot and save pie charts for all mouse groups in one figure."""
    num_groups = len(pie_chart_data)
    ...
    for ax, (group, counts) in zip(axes, pie_chart_data.items()):
        if counts.sum() == 0:
            continue
...
def plot_grouped_bar_chart(total_counts_per_cell_type, save_path):
    """Plot and save a grouped bar chart with mouse groups on x-axis and cell types as bars."""
    df = pd.DataFrame(total_counts_per_cell_type).T.fillna(0).astype(int)
...
def main():    
    ...
    for group, mice in mice_to_analyze.items():
        for mouse in mice:
            ...
            mouse_counts = cell_type_counts.iloc[:, :].sum().astype(int)
            total_counts_per_cell_type[group].append(mouse_counts)
    ...
    output_file = os.path.join(counts_folder,"test", "cell_type_totals.tsv")
    results_df.to_csv(output_file, sep='\t')
    print(f"Saved cell type totals to {output_file}")
    
    # Generate and save plots
    pie_chart_path = os.path.join(counts_folder,"test", "all_groups_pie_chart.png")
    bar_chart_path = os.path.join(counts_folder,"test", "all_groups_bar_chart.png")
    per_mouse_chart_path = os.path.join(counts_folder,"test", "per_mouse_group_bar_chart.png")
