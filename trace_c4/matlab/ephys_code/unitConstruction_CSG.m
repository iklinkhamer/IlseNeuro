...
function unitConstruction_CSG(kwargs)
arguments
    kwargs.convertAllSessions = true;
    kwargs.convertAllMice = false;
    kwargs.startMouse = 8;
    kwargs.skipToday = false;
    kwargs.directory = fullfile(Env.getBayesLabUserRoot, "/ContextMouseExperiments/Ilse/analysis_directories")%fullfile(Env.getBayesLabUserRoot, "/ContextMouseExperiments/Analysis/BEPAK/analysis_directories")
end
P = IkUtils.getParams();
getSessionsLater = false;
mouseCodes = [defaultMice_CSG().code];
mouseNames = [defaultMice_CSG().name];
...
if kwargs.convertAllSessions == 0
    ...
    getSessionsLater = true;
end


for mcode = mousecodes
    mname = mousenames(mousecodes==mcode);
    mdir = dir(fullfile(kwargs.directory, mname));
    ...
    for s = 1:length(sessions)
        session = sessions(s);
        folder = mfolders(s);
        fprintf("Mouse: %s, %s  Session: %d \n", mcode, mouseNames(mouseCodes == mcode), session);

        % if kwargs.skipToday
        %     [file_struct, folder_struct] = recursiveFileSearch('StructEphysData*.mat', mcode, session);
        % 
        ...
        %[S, cell_spk, stimtimes, path] = loadData(mcode, session);
        stimtimes_struct = load(fullfile(kwargs.directory, mname, folder, "Ephys analysis/EventStamps.mat"));
        stimtimes = [stimtimes_struct.EvTimeCS]';
        spiketimes = readNPY(fullfile(kwargs.directory, mname, folder, "/kilosort/spike_times.npy"));
        fr = 30000;
        spiketimes = double(spiketimes)/fr;
        clusters = readNPY(fullfile(kwargs.directory, mname, folder, "/kilosort/spike_clusters.npy"));
        behavior_file = dir(fullfile(kwargs.directory, mname, folder, "/Ephys analysis/ButtonPress*.csv"));
        if isempty(behavior_file)
            behavior_file = dir(fullfile(kwargs.directory, mname, folder, "/Ephys analysis/Trial_Parameters_*.csv"));
            behavior_data = readtable(fullfile(behavior_file.folder, behavior_file.name));
            light_dark = logical(behavior_data.Var2);
        else
            behavior_data = readtable(fullfile(behavior_file.folder, behavior_file.name));
            light_dark = logical(behavior_data.LightDark);
        end

        path = fullfile(fileparts(kwargs.directory), "CSG");
        light_tunnel_interval = 0.8;
        dark_tunnel_interval = 0.5;
        % if isempty(S) || isempty(cell_spk) || isempty(stimtimes)
        % end
        unique_clusters = unique(clusters);
        unit.neuron = [];
        bins = -P.t_raster_edges : P.spikeTimesBinW : P.t_raster_edges;
        n_neurons = length(unique_clusters);
        for i = 1 : n_neurons
            unit.neuron(i).spiketimes = spiketimes(clusters==unique_clusters(i));
            unit.neuron(i).irc_trueID = unique_clusters(i);
            unit.neuron(i).channels = [];
            unit.neuron(i).bins_cs = bins;
            unit.neuron(i).bins_us = bins;
            unit.irc_trueID(i) = unique_clusters(i);
        end

        unit.full_spiketimes = spiketimes;
        unit.full_clusters = clusters;
        unit.bins_cs = bins;
        unit.bins_us = bins;
        unit.mcode = mcode;
        ...
        % if all(diff_stim_times(2:2:end, 1) > 10) % If this is the case than only the stimtimes of the light have been recorded.
            ...
            stim_times.lightON = stimtimes(light_dark); %Light tunnel
            stim_times.lightOFF = stim_times.lightON + light_tunnel_interval; %Light tunnel intended end
            stim_times.CSonlyLightON = stimtimes(~light_dark); %Dark tunnel
            stim_times.CSonlyLightOFF = stim_times.CSonlyLightON + dark_tunnel_interval; %Dark tunnel inteded end

            stimtimesCS = stimtimes; % Take only the odd stimtimes, because those are the times that the light is turning ON.
            stimtimesCS(light_dark,2) = 1; %2025: --> light tunnel trials
            stimtimesCS(~light_dark,2) = 0; % IK: CS only trials. --> 2025: dark tunnel trials
            ...
            stimtimes(light_dark,2) = 1; % Times that the light goes on are 1. --> 2025: light tunnel trials
            stimtimes(~light_dark,3) = 1; % IK: CS only trials. --> 2025: dark tunnel trials
        ...
        %stimtimes_sorted = stimtimes';
        %stimtimesCS_sorted = stimtimes';
        %stimtimesCS_sorted(:,2) = 1;
        %stimtimesCS = stimtimes';
        ...
        for n = 1 : n_neurons
            ...
            [psth_cs_reset, psth_us_reset, psth_cs, psth_us] = calcPsthReset(unit, rasters, stimtimesCS_sorted);
            unit.neuron(n).psth_cs_reset = psth_cs_reset;
            unit.neuron(n).psth_us_reset = psth_us_reset;

            unit.neuron(n).psth_cs = psth_cs;
            unit.neuron(n).psth_us = psth_us;
            ...
            unit.neuron(n).psth_cs_filtered = psth_cs;
            unit.neuron(n).psth_us_filtered = psth_us;

        end

        % stamp = nameDateStampFiles(mcode = mcode, s = session, file_pattern = 'StructEphysData', file_extension = ".mat");
        stamp = sprintf("StructEphysData_%s_%s.mat", mname, folder);
...
function [psth_cs_reset, psth_us_reset, psth_cs, psth_us] = calcPsthReset(unit, rasters, stimtimes)
P = IkUtils.getParams();
for c = 1 : length(unique(stimtimes(:,2)))
    ...
    spike_rates_cs = spike_counts_cs/(unit.bins_cs(2)-unit.bins_cs(1));
    ...
    spike_rates_us = spike_counts_us/(unit.bins_us(2)-unit.bins_us(1));
    % create filter
    ...
    psth_cs(c, :) = spike_rates_cs;
    psth_us(c, :) = spike_rates_us;
end
end
...
trials = unique(floor(raster(2,:)));
trials = trials(~isnan(trials));
nTrials = length(trials);
% binEdges = range.min:binWidth:range.max;
P = IkUtils.getParams();
binEdges = -P.t_raster_edges : P.spikeTimesBinW : P.t_raster_edges;
filterMask = zeros(1,length(raster));
for t = 1 : nTrials
    trial = trials(t);
    spikeTimesTrial_ = raster(1,raster(2,:) == trial);
    rangeMask = ...
