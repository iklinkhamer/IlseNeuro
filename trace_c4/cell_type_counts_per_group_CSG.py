...
def main(results_file_or_folder=None, contamination_ratio=0.1, confidence_ratio_threshold=1.5, select_folder=False):
    ...
    for session_folder, session in zip(sorted(subfolders), sorted(sessions)):
    ...
    for ext in save_types:
        ...
        plot_pie_charts(results_all_mice_df, pie_path)
        plot_grouped_bar_chart(results_all_mice_df, bar_path)

    results_all_mice_df.to_csv(os.path.join(
        counts_folder,
    ...
    grouped_df = summary_df.groupby("base_name")[cell_type_names].sum().T
    ...
    for ext in save_types:
        ...
        plot_pie_charts(grouped_df, pie_path)
