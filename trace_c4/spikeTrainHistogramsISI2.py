...
def compute_autocorrelogram(spike_times, spike_clusters, bin_width=1.0, max_lag=50, save_diffs_path=None):
    ...
    if save_diffs_path and os.path.exists(save_diffs_path):
        ...
        for i in tqdm(range(len(spike_times)), desc="Processing spikes"):
            diffs = spike_times - spike_times[i]
            spike_diffs.extend(diffs[np.abs(diffs) <= max_lag])  # Keep only diffs within max_lag
...
if __name__ == "__main__":
    ...
    try:
        ...
        save_diffs_path = f"{directory}isi_values/spike_differences.npy"
